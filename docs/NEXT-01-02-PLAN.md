...
| J02 | Concrete leaf record types/validators preserve hashes, identities, policy and nullable observations. | Partial: concrete leaf/report/authority/latch plus dispatch/decision returns; original public input objects and serialization retained. Sparse slots and inherited declared fields reject. Production usage/observation validation is now delivered in the AR-01 source checkpoint. Contracts/schema/observations have zero supplemental compiler diagnostics; see the stored-record runbook for toolchain limits. |
| J03 | Aggregate current/legacy contracts enforce consumed-field/reference invariants without activating V2. | Source implemented: [AR-01 checkpoint](STORED-RECORDS-PLAN.md#9-ar-01-source-checkpoint) records concrete current/historical aggregates, pure non-authorizing classification, compatibility corrections and strict profile selection. Intended-toolchain gate remains blocked; no V2/runtime integration. |
| J04 | Pure transition event union covers closure, holds, waits, duplicates, delivery and uncertainty. | Source implemented: §10 records 27 event kinds with modeled dispatch, continuation, evidence, decisions, notices, accounting, amendments, reconciliation and reset. Independent review corrections integrated; all outcomes remain non-authorizing. Unsupported variants and runtime qualification remain open; no Controller/Worker wiring. |
| J05 | Frozen model and integration handoff identify every unconverted Controller/Worker path. | Internal model and consumer handoff recorded in §10; canonical genesis/journal and checked projections are not a disk format. Config V3/State V2/control-wire V2 remain AR-03 targets requiring complete reader/writer freeze before AR-04 activation. Trusted producers and durable history/effect protocols are not implemented. |
| J06 | Static results and remaining risks are recorded without recreating tests or claiming release qualification. | §9 preserves the earlier 538-diagnostic checkpoint. §10 records the current kernel source review and no-file observations; syntax/whitespace pass, supplemental TS6 full source remains 554 source/0 dependency diagnostics with zero kernel diagnostics. Pinned toolchain remains unavailable. No tests or qualification claim. |
...
**Historical initial checkpoint.** The source descriptions and compiler counts in this section describe the original 510-line slice. AR-01 superseded its stored-record gaps; §10 supersedes its kernel coverage. Retain this evidence as history, not current status.
...
## 10. NEXT-02 transition-model source checkpoint

**Scope:** working-tree implementation against `078820d2c94057dfa9a808054a2890559e494f96`, following AR-01. Production source changes in this batch are limited to the unused `src/coordination.js` and `src/transitions.js` modules; no Controller, Worker, RPC, configuration, active storage or wire integration was made. Astra supplied design, implementation, correction and independent reviews; a Zro DeepSeek V4.1 Flash source inventory succeeded after enabling provider extensions in the retry child. Earlier inventory failures were model-admission failures, not source findings.

### Internal contract and event coverage

`validateCoordinationModel` now accepts `{mode: 'internal-non-authorizing', genesis, events}` or that history plus **all** replay projections. Genesis contains the receiving fence, nullable held-legacy record and explicit unresolved holds. Retained events are canonical; supplied projections must equal replay. This replaces the old unused singleton-attempt input, not any saved V1 state. Every event has a stable observation ID, receiving fence and observation time. Revision references are opaque checked IDs, not fabricated monotonic storage transactions.

Collections retain tasks, grants, prompt deliveries, full reports, immutable decisions, notices, intents, commitments, receipts, scoped hold causes, accounting and reset history. Task obligations distinguish a report awaiting decision from its committed continuation; cancellation records the disposition of a not-yet-issued continuation without deleting its decision or physical obligations. Grant closure is absorbing. Current-owner receipts about historical producers never rebase those producers' identities.

| Event kinds / issuer boundary | Modeled result |
|---|---|
| `dispatch-requested` / controller; `intent-committed` / storage | Retain exact assignment intent before commitment. Public request keys remain reserved even when capacity holds the request. Queued intent grants no work. Old-fence commitments remain evidence, not actionable current commitments. |
| `prepare-grant`, `grant-committed`, `authority-published`, `dispatch-prepared` / assignment, storage, publication and dispatch boundaries | Distinct grant/prompt identities; fresh attempts and leases; committed dispatch/decision/recovery basis; immutable admission snapshots. A held, unprepared grant may explicitly refresh accounting/budget references through `refreshOfObservationId`, retaining the same operation/identity/basis and every prior receipt. It cannot refresh or reopen a prepared/closed grant. |
| `report` / worker-report boundary | Retain original payload, hash, producer identity and notice. `maxReportBytes` bounds UTF-8 **payload** bytes, not the envelope. Matching historical reports survive rotation/reset; contradictory late evidence holds and closes current admission without overwriting its obligation. |
| `effects-reconciled`, `checkpoint-finalized` / effect and evidence boundaries | Only a current complete barrier covering every cumulatively known admitted effect supports finalization/containment. Later uncertainty invalidates older complete barriers. Verification results are bound to configured commands and checkpoint snapshots. |
| `evidence-receipt`, `checkpoint-current`, `decide` / inspection, evidence and Main decision boundaries | Retain reviewer-bound evidence and operation-scoped checkpoint witnesses. Only a committed immutable decision consumes a report and creates a continuation or terminal outcome; approval advances at most once. Current bounded decision issuer is Main, not an activated supervisor. |
| `notice-resolved`, `main-delivery` / workflow and notice boundaries | Publication, notification, observation and decision-bound resolution are independent facts. No callback implies observation; notice resolution does not settle execution. |
| `worker-delivery`, `runtime` / correlated command, run and effect boundaries | Send, ACK/rejection, start, run settlement and effect settlement remain distinct. Start/settlement can arrive before ACK without fabricating acceptance. Contradictory historical execution closes replacement admission as well as retaining the old fact. Idle/stopped is not a barrier. |
| `usage-observed`, `counter-observed`, `budget-evaluated` / accounting and clock boundaries | Deduplicated lifetime/per-step counters and fractional usage; explicit completeness gaps. Implementer attribution uses one complete grant/delivery tuple; null step attribution remains unknown. Below-budget accounting gaps block future admission, not ordinary turns already admitted. Late counters remain uncharged reconciliation evidence. Known exhaustion closes admission. |
| `amendment-proposed` / explicit user authorization | Immutable genesis plus committed, whitelisted amendments; never configuration copy-in. No consumption is refunded, and resume never resets policy, counters or deadlines. |
| `automatic-action-prepared`, `reconciliation-proposed` / workflow and reconciliation boundaries | Model bounded per-task repair/recovery reservations and evidence-backed command/publication/accounting/report-adoption findings. No executor, automatic retry, repair loop or queue pump is enabled. Unknown outcomes do not justify prompt replay. |
| `reset-requested` / user plus archive/storage boundaries | Detach only after obligations, accounting and containment prerequisites and matching archive coverage; retain receipts, reports, decisions, counters and deliberate stop/disable holds. |
| `lifecycle`, `owner-replaced`, `failure`, `unsupported` | Independent holds, explicit cancellation, owner/generation fencing, retained failures and conservative unsupported outcomes. Resume restores a known wait/continuation; it creates no new grant. |

The original three exports remain: `validateCoordinationModel`, `validateTransitionEvent`, `reduceCoordination`. Added `CoordinationValidationError` distinguishes expected data/history rejection from programming errors. All reducer results carry `nonAuthorizing: true`. The facade catches only that expected error; generic `Error` remains indistinguishable from expected schema validation **only inside the two existing public schema-validator calls**, not throughout replay.

### Budget, scope and version decisions

- Assignment commitment anchors the existing wall-clock task budget; active-step time is separately accumulated from identified monotonic intervals. Queue/review/permission/pause waits are not active time. Lifetime and per-step counters survive attempts and resume.
- Zero review capacity admits no work needing a report reservation. Limit reductions never evict retained obligations. Repair/recovery allowances are lifetime-per-task model counters; no automation is run.
- An explicit amendment may change only the checked limit/revision/summary fields or absolute lifetime deadline. Review mode, required final review, verification commands, workspace, role and permissions are not amendable through this event.
- Package/config/state/wire remain **0.1.0 / V2 / V1 / V1**. AR-03 still owns the full proposed **Config V3 / State V2 / actor-control wire V2** reader/writer freeze. These are separate format versions, not product V2/V3. Activation requires AR-04's coherent migration/publication rollout; V1 peers cannot silently ignore required V2 grant proof.
- The internal journal/projections are a model handoff, **not** the finalized State V2 layout or a second mutable actor registry. There is no active legacy-model adapter, disk migration, supervisor activation or registry/configuration entry in this slice.

### Runtime producer/consumer handoff

This table records the pre-AR-02 model checkpoint. AR-02 subsequently changed lifecycle and report retention, not kernel enforcement; use the [current AR-03 reconciliation map](AR-03-CONTRACT-FREEZE-PLAN.md#21-reuse-table-and-specific-gaps) before implementing adapters.

| Existing mutation path | Required later integration, not delivered here |
|---|---|
| `PairController.dispatchUnlocked`, `activate`, `rotateAttempt`, `writeAuthority`, `persist` | Durable dispatch/grant/prompt intent and publication/readback; exact command correlation; one effect executor after readiness revalidation. |
| `acceptReport`, `finalizeReport`, `inspect`; Worker `pair_report`/latch/outbox | Retained ingress, admitted-effect barrier and per-command snapshot evidence; bound inspection delivery. Neither `agent_settled` nor the current scalar tool/idle state supplies that barrier. |
| `decideUnlocked`, `deliverNotice`, `inbox` | Committed decision plus independent continuation/notice observations. Native notice stages are `published`, `notified`, `observed` in this model—not an inferred model-read acknowledgement. |
| `scan`, `normalizedUsage`, `addUsage`, `resume` | Trusted attribution/dedup and completeness reconciliation; no continuation-time turn reset, resume-time deadline reset or current-config substitution. Main/warming accounting remains separate; supervisor usage is unsupported until a correlated producer exists. |
| `init`, `interrupt`, `pauseUnlocked`, `cancelUnlocked`, `stopUnlocked`, `reset`; Main binding/navigation | Preserve pending reports and original bytes, fence old effects, reconcile ownership and archive before detaching. The current runtime's identity fabrication and pending-report clearing remain unchanged hazards. |

### Acceptance evidence and open limits

- Source review identified and corrected stale-intent admission, forgotten effect obligations, historical-report loss, ordinary-usage dead ends, stranded cancellation, unrecoverable request holds, split producer attribution and error masking. A second review caught public-key reservation, held-grant refresh and historical-execution containment defects; parent corrected these and inspected the affected paths directly.
- Changed-module syntax and whitespace checks pass. Supplemental **TypeScript 6.0.3 / Node typings 26.6.2**, all **16 production roots**, existing strict options with external in-memory resolution: **exit 2, 554 source / 0 dependency diagnostics; both kernel modules 0**. This is not the missing pinned-toolchain gate, which remains open; no install, suppressions or exclusions were added.
- Narrow no-file production API observations showed start-before-ACK with no fabricated ACK; two below-budget turns retaining incomplete accounting; an 84-byte payload admitted despite a larger envelope; forgotten-effect rejection and renewed barrier uncertainty; held public-key conflict; explicit grant snapshot refresh preserving the old receipt; and closure of a replacement grant on late historical execution. Implementer observations additionally exercised decisions/continuation, cancellation/archive detach and reconciliation. These are not exhaustive transition coverage or runtime qualification, and no tests, fixtures, reusable runners or generated evidence files were created.
- Mechanical inspection found only `transitions.js` importing the kernel: no Controller/Worker/Main consumers, new Pi tools/commands, public schema/configuration entries or active version changes.
- **Still unsupported/blocked:** supervisor invocation/usage correlation, arbitrary plan replacement, unsafe legacy adoption, unproven accounting/settlement and a disposition protocol for contradictory historical reports. Such reports are retained and held, not silently resolved. Late counters require reconciliation. Journal replay/retention cost and eventual durable journal bounds/archival are integration concerns, not a claimed production-ready storage design.

**Historical next step at this model checkpoint:** AR-02 one-runtime extraction/protocol correction. Its source slice is now delivered; the current next batch is the [AR-03 contract-freeze plan](AR-03-CONTRACT-FREEZE-PLAN.md), followed by AR-04's coherent host/recovery integration. Full NEXT-01/02 foundation, intended-toolchain checking and original behavioral/native/release gates remain open. No tests or runtime integration are authorized by this model checkpoint or the AR-03 planning document.
