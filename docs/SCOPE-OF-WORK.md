...
**Execution plan:** [current no-tests implementation sequence](IMPLEMENTATION-PLAN.md#13-active-implementation-plan-after-test-removal).
...
Historical checkpoints recorded 55, 59, 81 and finally 86 offline passes plus five narrow native scenarios. Their suites, runners and artifacts have since been removed and those counts are not current evidence. The latest combined implementation compiler run exits 2 with 538 Pair-source and zero dependency diagnostics, down from the 620-source baseline. Retained tooling is `typecheck` and `pack:check`, not automated behavioral acceptance. The supplied Pi/Fabric/Fovea reference sources remain outside the edit scope.
...
**Next bounded implementation target:** execute [NEXT-01–06](IMPLEMENTATION-PLAN.md#13-active-implementation-plan-after-test-removal): concrete records and transition rules, held state-V2 migration, durable grant/report/import paths, complete production typing and static-only integration. No tests, queues, automatic repair/recovery, parallel workers, new transport or paid inference are added. Source review and static checks do not close the original behavioral gates.
...
| Product | V1, incomplete | One retained worker and one serialized Main review flow. |
...
Named module/symbol witnesses below include the first NEXT-01/02 implementation slice. Other line ranges refer to the earlier planning snapshot; no runtime migration or Controller/Worker transition integration has been activated.

| Current source witness | Gap / required treatment | Next owner |
|---|---|---|
| `src/contracts.js`: `validateStoredTask`, `validateWorkerRecord`, `validateStoredState` | Leaf/report/authority/latch and public schema returns are now concrete; aggregate stored records remain generic. Nested usage/observations, legacy classification and consumed-field/reference coverage remain incomplete. | NEXT-01 / B4 |
| `src/coordination.js`, `src/transitions.js`; scattered Controller/Worker mutations remain | Bounded single-assignment kernel now exists with non-authorizing outcomes. Dispatch/continuation/checkpoint/decision/reset/accounting/reconciliation families still hold as unsupported. No runtime consumers or completed model freeze. | NEXT-02 / B5, then NEXT-03/04 |
| `src/contracts.js`: `migrateStoredState`; `src/controller.js:26–46` | V1 migration creates missing attempt identity; startup clears pending reports and persists the migrated state without retained original-state backup. Use truthful held legacy records, preserve bytes/obligations and reject corrupt input. | NEXT-03 / B4 |
| `src/controller.js:281–302`; `src/util.js`: `atomicJSON` | Running authority is published before state persistence; per-file sync/rename is not a durable multi-file grant transaction. Implement committed grant identity and startup reconciliation. | NEXT-04 / B2+B6 |
| `src/worker.js:74–109`; `src/controller.js:310–410,484–521` | Latch precedes inbox publication; an identical retry can return from the latch without fixing failed delivery. Interrupt/restart/stop can discard pending obligations. Reconcile reports/notices/archive without blind mutation replay. | NEXT-03/04 |
| `src/config.js:252–264` | Migration rollback errors are swallowed. Preserve recoverable backups and report primary plus rollback failure without overwriting changed files. | NEXT-04 / B6 |
| `src/controller.js:491–501` | Resume copies current policy/limits, resets turn/time fields and supersedes notices. Held state must be blocked in the foundation; complete policy-preserving lifecycle integration is later IMP-05. | NEXT-02/03 guards; later IMP-05 |
| `tsconfig.json`, `package.json`, last compiler result | All production JS remains included; latest compiler: 538 source/0 dependency diagnostics, exit 2. Contracts/schema/coordination/transitions have zero diagnostics. Only `typecheck`/`pack:check` are registered. | NEXT-05/06 / B7+B8 implementation portion |
...
**Order:** NEXT-01 + NEXT-02 → NEXT-03/04 coherent integration → NEXT-05 → NEXT-06. Type annotations accompany every batch. Shared controller/worker edits are sequential. Full lifecycle/admitted-effect/evidence safety remains IMP-05–08, before report limits, queues and automation.

**First joint batch:** use [NEXT-01-02-PLAN.md](NEXT-01-02-PLAN.md). Its J01–J06 slices define the concrete record inventory, pure transitions, compatibility decisions and static handoff. The immediate [stored-record/classification plan](STORED-RECORDS-PLAN.md) narrows the next implementation to typed aggregates, source-backed legacy profiles and pure classification, with no runtime migration or transition expansion. Active state/wire versions stay at V1 until the later coordinated migration/publication rollout; a model-ready helper is not yet runtime enforcement.
