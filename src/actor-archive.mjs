...
export function snapshotArchiveFiles(value) {
...
export const archiveHash = (bytes) => createHash('sha256').update(bytes).digest('hex');
const same = (a, b) => encodePairJSON(a) === encodePairJSON(b);
export function emptyArchiveContext(storeId) {
    return validateArchiveValidationContext({ version: 2, kind: 'empty', storeId, head: null, segments: [], artifacts: [] });
}
export async function readArchiveContext(value, read) {
    const root = decodePairJSON(encodePairJSON(value));
    ...
    while (true) {
        ...
        const decoded = decodePairJSON(original);
        ...
        if (decoded.genesis.checkpoint === null)
            break;
        checkpoint = validateArchiveCheckpointV2(decoded.genesis.checkpoint);
    }
    for (const reference of retained.values())
        artifacts.push({ reference, original: await load(reference.ref, COMMON_BOUNDS.maxReferenceBytes) });
...
function preserved(before, after) {
    for (const key of ['actor', 'workflows', 'mailboxes', 'accounting', 'implementation'])
        assert(same(before[key], after[key]), `Archive changed ${key} projection`);
}
export function planArchiveRotation(rootBytes, context, proposedCheckpoint) {
    ...
    return Object.freeze({ nonAuthorizing: true, model, context: nextContext,
...
export function validateArchiveSuccessor(prior, next, context) {
    const after = validateActorStateV2(next, context);
    const before = decodePairJSON(encodePairJSON(prior));
    if (before.segmentId === after.state.segmentId) {
        ...
        for (let i = 0; i < before.events.length; i++)
            assert(same(before.events[i], after.state.events[i]), 'Actor event prefix changed');
    }
    else {
