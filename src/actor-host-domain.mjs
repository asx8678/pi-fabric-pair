...
const same = (a, b) => encodePairJSON(a) === encodePairJSON(b);
const detach = (value) => decodePairJSON(encodePairJSON(value));
export function createActorHostDomain(options) {
    const input = detach(options);
    ...
    let phase = 'new';
    let capability = null, selected = null, model = null;
    let context = emptyArchiveContext(storeId);
    let chain = Promise.resolve(), pending = null, generation = 0, trulyAbsent = false, captures = false;
    ...
    const revoke = () => { generation++; if (phase !== 'released')
        phase = 'held'; };
    ...
    const resolve = (value) => withDirectory('/', async () => {
        const budget = readBudget();
        return readArchiveContext(value, (ref, maximum) => readBounded(path.join(storeRoot, ref), maximum, budget));
    });
    const adopt = async (readback) => {
        if (readback === null) {
            selected = null;
            model = null;
            context = emptyArchiveContext(storeId);
            return;
        }
        ...
        let nextModel = null, nextContext = emptyArchiveContext(storeId);
    ...
    const publish = (nextModel, nextContext, operation) => {
    ...
    return Object.freeze({
        ...
        acquire() {
            ...
            return serialize(async () => {
                try {
                    ...
                    if (generation !== epoch || phase !== 'acquiring')
                        return Object.freeze({ state: 'held', nonAuthorizing: true, issue: 'Host revoked during acquisition; reservation retained' });
                    if (capability === null) {
                        phase = 'held';
                        return { state: result.state, nonAuthorizing: true, issue: result.issue };
                    }
                    await adopt(await Store.readHead(storeRoot));
                    if (generation !== epoch || phase !== 'acquiring')
                        return Object.freeze({ state: 'held', nonAuthorizing: true, issue: 'Host revoked during adoption; reservation retained' });
        ...
        transition(expectedValue, event) {
            ...
            if (result.kind === 'reject' || result.kind === 'noop' || same(result.model.state, model.state))
            ...
            for (const entry of result.model.events)
                if (entry.domain === 'actor' && entry.payload.kind === 'owner-transitioned')
                    currentOwner = entry.payload.next;
            assert(same(currentOwner, owner), 'Owner change requires a new Store ownership protocol');
            return publish(result.model, context);
        },
        planArchive(expectedValue, checkpoint) {
            checkExpected(expectedValue);
            quiescent(model);
        ...
        archive(expectedValue, checkpoint) {
            checkExpected(expectedValue);
            quiescent(model);
            assert(selected?.rootBytes !== null && selected?.rootBytes !== undefined, 'Selected root bytes required');
        ...
        release() {
            ...
            return serialize(async () => {
                ...
                if (owned === null || pending !== null || captures)
                    return Object.freeze({ state: 'held', released: false, retained: true });
                try {
                    ...
                    try {
                        ...
                        if (proof !== undefined)
                            discardHostReleaseHandoff(proof);
                    }
                }
                catch (error) {
                    return Object.freeze({ state: 'held', released: false, retained: true, issue: error instanceof Error ? error.message : String(error) });
                }
            });
        },
        ...
        heldCapture(captureId, sourceBytes) {
        ...
        inspectHeld: (workerId) => serialize(() => Store.inspectHeld(storeRoot, workerId)),
    });
}
